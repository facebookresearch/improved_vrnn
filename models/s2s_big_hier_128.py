...
"""
    ...
    v3 changes:
        (overriden, back to 1-8-32) Latents at 1-4-8-16-32

    v4 changes:
        Shared encoder for everything

    128 version of the shared encoder model
"""
# Imports ---------------------------------------------------------------------
import torch
...
class Model(nn.Module):
    ...
    def __init__(self, img_ch, n_ctx,
        ...
        nn.init.constant_(self.posterior_branches['layer_19'][-1].model.weight, 0)
        nn.init.normal_(self.posterior_branches['layer_19'][-1].model.bias, std=1e-3)
        ...
        nn.init.constant_(self.prior_branches['layer_19'][-1].model.weight, 0)
        nn.init.normal_(self.prior_branches['layer_19'][-1].model.bias, std=1e-3)
            ...
            for branch_layer_idx, layer in enumerate(branch_layers):
                ...
                # Handcrafted rules for integrating the different z's
                    ...
                    if layer_idx == 19:
                        ...
                        z1 = F.interpolate(z1, scale_factor=16)
                        ...
                        b, t, c, h, w = z1.shape
                        z1 = z1.view(b*t, c, h, w)
                        z1 = F.interpolate(z1, scale_factor=64)
                        z1 = z1.view(b, t, c, z1.shape[-2], z1.shape[-1])
                        b, t, c, h, w = z2.shape
    ...
    def forward(self, frames, config, use_prior, use_mean=False):
        ...
        q_dists = self.posterior(emb, emb, use_mean=use_mean)
        p_dists = self.prior(emb, emb, q_dists, use_prior, use_mean=use_mean)
